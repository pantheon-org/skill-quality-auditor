...
func TestScoreFullAGrade(t *testing.T) {
	...
	// D6 re-baselined: new sub-component scorer replaces single InstructionSpecificity expression.
	// skill-full now includes soft-default markers (PREFER/AVOID/TYPICALLY) scoring 13/15 on D6.
	if result.Total < 119 {
	...
	if GradeRank[result.Grade] < GradeRank["B+"] {
		t.Errorf("skill-full expected grade B+ or higher, got %s (total %d)", result.Grade, result.Total)
	}
}
