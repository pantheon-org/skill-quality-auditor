...
**WHY:** Skills must be fully self-contained. When installed via a tile/package manager (e.g. `tessl install` or `npx skills add`), they land in arbitrary directories — any reference to files outside the skill's own directory tree will break.
