...
1. **A new tracked sidecar, `scripts/docs-drift-reviewed.jsonl`, records per-doc review baselines** — one JSON object per line, `{"doc": "...", "reviewed_iso": "...", "reviewed_epoch": N}` — separate from `check-docs-drift.sh`'s existing `MAPPINGS` array (which encodes doc-to-source-glob relationships and changes far less often than review timestamps do). A new helper, `scripts/mark-docs-reviewed.sh <doc_path>...`, writes to it via `jq`, already an assumed-available dependency elsewhere in this repo's `scripts/` (`plumber-*.sh`).
...
5. **No JSON-Schema-validated template for the sidecar.** This repo's template+schema+validation-script convention governs skill artifacts under `.context/plugins/`; a small JSONL file of internal tooling state isn't that kind of artifact, and a schema for it would be disproportionate to the fix's scope.
6. **No bash associative arrays (`declare -A`) anywhere in `check-docs-drift.sh`.** Discovered during implementation, not anticipated by the plan review: `env bash` on this repo's own dev machine resolves to macOS's stock `/bin/bash` 3.2.57, which predates bash 4 and has no associative arrays, despite a newer bash being installed elsewhere on the same machine but not first in `PATH`. Since this script runs on contributors' local Macs via the pre-push hook, not just CI, an associative-array-based sidecar preload would break for likely-common local setups. Fixed with a per-doc `jq` lookup on each `MAPPINGS` iteration instead of a preloaded map — no bash-version dependency.
