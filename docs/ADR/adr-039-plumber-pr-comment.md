...
1. **`plumber.yml` gets a comment step, `pull_request`-only** (`if: always() && github.event_name == 'pull_request' && ...`), summarizing the Critical gate's pass/fail state and, when present, the High/Medium/Low counts on this branch with a link to the persistent rollup issue (ADR-038) — noting explicitly that the rollup issue reflects `main`, not this PR's branch, since the two can diverge until merge.
