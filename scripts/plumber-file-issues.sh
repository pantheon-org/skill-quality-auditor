...
# body from the current results.json: creates the issue if none exists,
# edits it in place if open, reopens + edits it if a prior run closed it.
# If a run has zero High/Medium/Low findings and the issue is open, it is
# closed as resolved. This never creates more than one open issue.
#
