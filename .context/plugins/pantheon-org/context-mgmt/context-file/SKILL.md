...
- `deferred_until` — optional reactivation date (`YYYY-MM-DD`) for a **date-gated** `DEFERRED` item: the date on or after which it should be reconsidered. Only valid with `status: DEFERRED`. Omit for externally-blocked items with no known ripen date. The read protocol does **not list** an item whose `deferred_until` is still in the future — it is hidden from the pick until then, and surfaces as reactivation-eligible once the date passes. The date takes precedence even if the item is also blocked.
