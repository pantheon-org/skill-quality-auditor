...
- **Observed false positive (2026-07-06, PR #187):** `/review`'s ticket-compliance sub-feature (`require_ticket_analysis_review`, on by default) graded PR #187 against PR #186's own description, triggered by a casual "closing out #186" prose reference auto-linked by GitHub — not an actual tracked-ticket link, since this repo uses `.context/plans/`/`.context/findings/` rather than GitHub Issues for that role. See `.context/findings/pr-agent-ticket-compliance-false-positive-2026-07-06.md`. Fixed by setting `require_ticket_analysis_review = false` in `.pr_agent.toml` — not counted against `/describe`+`/review`'s core signal quality, since it's a config mismatch with this repo's workflow, not a judgment error by the LLM itself.
